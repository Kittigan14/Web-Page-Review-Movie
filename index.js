...
app.get('/detailMovie/:MovieID', async (req, res) => {
    try {
        ...
        res.render('DetailMovie', { ...response.data, userName, movieId });
    } catch (error) {
        console.error(error);
        res.status(500).send('Internal Server Error');
    }
...
app.get('/reviews', async (req, res) => {
    ...
    res.render("DetailMovie", { userName: userName, movieId: movieId });
});
...
app.get('/profile', async (req, res) => {
    try {
        ...
        if (response.data) {
            ...
            res.render('Profile', { userName, profileData });
        } else {
...
app.post('/deleteUser', async (req, res) => {  
    try {
        ...
        await axios.delete(`${base_url}/deleteUser`, { data: { userName } });
        req.session.destroy();
        res.redirect('/');
    } catch (error) {
        console.error(error);
